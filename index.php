...
if ($hal == ""){$hal = "1";}
  $xfile=@file("arsip/art.txt");
  $first = count($xfile) - ($post * ($hal - 1));
  $second = count($xfile) - ($post * $hal) + 1;
if ($second < 1) {$second = 1;}
  $hals = (count($xfile) / $post);
if ($hal>1) $rew = "<a class=\"btn btn-primary btn-xs\" href=\"./?hal=".($hal-1)."\">« Previous</a>";
if ($hal<$hals) $next = "<a class=\"btn btn-primary btn-xs\" href=\"./?hal=".($hal+1)."\">Next »</a>";
for ($i = $first-1; $i >= $second-1; $i--) {
$ii = $i;
$ii++;
$data = explode("|",$xfile[$i]);
$fkom=file("./arsip/kom_$data[0].txt");
print "
<span class=\"label label-default\">
<i class=\"fa fa-calendar-o\"></i> $data[1] <i class=\"fa fa-clock-o\"></i> $data[2]</span> <span class=\"label label-primary\"><i class=\"fa fa-comments-o\"></i> ".count($fkom)." comments.</span>
